    ...
    <div className="livingroom-container">
      ...
      {!showPopup && (
        <div className="livingroom-content">
          {level <= 4 ? (
            <h3>
              The living room, draped in a veil of twilight, exudes an
              unsettling calm. Faded, threadbare sofas face a cold, empty
              fireplace, while dusty portraits seem to watch with unseen eyes.
              The heavy air is filled with the faint creak of old wooden
              floorboards, and the faint scent of aged leather lingers. {name}{" "}
              feels a shiver as if the room holds memories of ghostly
              conversations and untold stories.
            </h3>
          ) : (
            <h3>
              Upon examining the fireplace in the living room, {name} discovers
              a small, concealed drawer behind the hearth. Inside, they find an
              ornate key, its intricate design matching the symbol seen on the
              iron gate. With the key in hand, {name} returns to the hall. The
              heavy iron gate now clicks open smoothly as the key turns in the
              lock. The path to the second floor is finally revealed, and the
...
const FirstTest = ({ name, win, onClose }) => {
  ...
  const quizQuestions = [
    {
      question:
        "How do you use useEffect to perform a side effect only when a specific prop changes?",
      options: [
    ...
    {
      question:
        "What is the syntax for using useEffect to run code both after the initial render and on every update?",
      options: [
        "useEffect(() => { /* code */ })",
        "useEffect(() => { /* code */ }, [])",
    ...
    {
      question:
        "How would you use useEffect to debounce a function call based on user input?",
      options: [
    ...
    {
      question:
        "What is the correct way to handle asynchronous code within useEffect?",
      options: [
